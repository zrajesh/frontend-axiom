...
Compare the two and report concrete deltas, not vague impressions: exact spacing differences, color hex mismatches, font-size/weight mismatches, alignment/gap differences. Reference `knowledge/css.md` for how the project's chosen styling approach expresses tokens (Tailwind scale, CSS variables, etc.) so fixes match the existing convention rather than introducing one-off values.
