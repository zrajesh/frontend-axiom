...
- **Destructuring rule**: grep for chained property access (`\w+\.\w+\.\w+` patterns, repeated `base.prop` on the same base within a scope) — flag violations with file:line. **A passing lint run is not sufficient evidence here.** The rule can't see single deep reads used once, and it can't see referential instability at all — so specifically check for object/array destructuring defaults (`= []`, `= {}`) that feed a dependency array, a memoized child, or a context value (`knowledge/principles.md` §3). Conversely, don't flag a discriminated union read after a narrowing check — that's a documented exception, and destructuring it would break TypeScript narrowing.
