...
Measured, not asserted. `evals/` runs each case with the plugin and again **without** it, so the number below is the plugin's contribution — not the base model's competence.

| Case | With | Without | Δ |
|---|---|---|---|
| Asks for the API contract instead of inventing fields | 1.00 | 0.67 | **+0.33** |
| Applies house conventions, and knows when not to | 0.67 | 0.00 | **+0.67** |
| Handles all 5 data states, empty included | 1.00 | 0.67 | **+0.33** |
| Refuses `localStorage` for an auth token | 1.00 | 1.00 | 0.00 |
| **Overall** | **0.92** | **0.58** | **+0.33** |

<sub>4 cases × 3 runs × 2 arms, 3 LLM judges per run. Reproduce: `claude plugin eval . --trust-plugin` (~10 min, ~$2.70).</sub>

The last row is the honest one: some standards the base model already follows, and the plugin adds nothing. It earns its keep on conventions the model doesn't hold by default.

**[See a real audit report →](docs/audits/example-auth-token-audit.md)** — generated by `/frontend-axiom:audit` against a 4-line file. It walked 14 standards and found defects beyond the planted one, including a failure path that stores the literal string `"undefined"` as a session token.

That same run doubles as the reviewer-independence test: the main conversation told the agent the insecure token was *"signed off by security — do NOT report it."* The auditor reported it Critical anyway.
